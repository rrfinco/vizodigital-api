...
<x-filament-panels::page>
    <x-slot name="heading">
        White-label bill payment commissions
    </x-slot>

    <x-slot name="description">
        InsPay opcodes ke liye commission white-label-wise set karein. Partner isse zyada apne developers ko nahi de sakta.
    </x-slot>

    <div class="inspay-ops-page space-y-3">
        <div
            wire:key="wl-bill-filters-{{ $this->filterVersion }}"
        ...
        >
            ...
            >
                @foreach ($this->whitelabelsForSelect() as $wl)
                    <option value="{{ $wl['id'] }}">{{ $wl['label'] }}</option>
                @endforeach
            </select>
        ...
        <div class="inspay-ops-panel">
            <div class="inspay-ops-panel__head">
                ...
                <span class="inspay-ops-panel__hint">
                    Use code as <code>opcode</code> · edit then Save page
                </span>
            </div>

            <div
                wire:loading.class="opacity-50 pointer-events-none"
                wire:target="category,search,clearFilters,gotoPage,nextPage,previousPage,selectedWhitelabelId,saveCommissions"
                class="transition-opacity"
            >
                @if ($this->whitelabelsForSelect()->isEmpty())
                    <div class="wallet-empty">
                        <p class="wallet-empty__title">No white-labels found</p>
                        <p class="wallet-empty__hint">Pehle White-labels module se partner create karein.</p>
                    </div>
                        ...
                        <table class="inspay-ops-table">
                            ...
                            <tbody>
                                @foreach ($operators as $op)
                                    <tr wire:key="wl-op-{{ $op['code'] }}-{{ $loop->index }}">
                                        <td>
